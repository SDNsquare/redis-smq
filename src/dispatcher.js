...
module.exports = function dispatcher() {
  ...
  function registerRuntimeEvents(dispatcherInstance) {
    ...
    if (dispatcherInstance.isConsumer()) {
      ...
      instance.on(events.MESSAGE_EXPIRED, message => {
        ...
        garbageCollectorInstance.collectExpiredMessage(message, keys.keyQueueNameProcessing, () => {
      ...
      instance.on(events.MESSAGE_CONSUME_TIMEOUT, message => {
        dispatcherInstance.handleConsumeFailure(message, new Error(`Consumer timed out after [${consumerMessageConsumeTimeout}]`));
      });
    }
  }
  ...
  function shutdownEventsHook(dispatcherInstance, event) {
    ...
    if (dispatcherInstance.isConsumer()) {
      if (
        shutdownFiredEvents.includes(events.GC_DOWN) &&
    ...
    } else if (shutdownFiredEvents.includes(events.SCHEDULER_DOWN) && (!statsInstance || shutdownFiredEvents.includes(events.STATS_DOWN))) {
      down();
    }
  }
  ...
  function startupEventsHook(dispatcherInstance, event) {
    ...
    if (dispatcherInstance.isConsumer()) {
      if (
        startupFiredEvents.includes(events.GC_UP) &&
    ...
    } else if (startupFiredEvents.includes(events.SCHEDULER_UP) && (!statsInstance || startupFiredEvents.includes(events.STATS_UP))) {
      up();
    }
  }
  ...
  return {
     ...
     * @param options
     */
    bootstrapConsumer(inst, cfg, options, queu) {
